...
class Metrics:
    ...
    def calculate_metrics(self):
        ...
        recall_file_path_define = self.recall_set_path
        for number in range(5):
            print("NUMBER: ", number)

            self.number_of_calculation = number
            output_file_path = f"{output_file_path_define}/cOS_{self.generator_name}_{self.type_cluster}_{self.number_of_calculation}_one_column.csv"

            recall_file_path = f"{recall_file_path_define}/cRS_{self.receptor}_{self.type_cluster}_{self.number_of_calculation}.csv"
