...
def plot_trends(df, type_split, type_scaf, subset, receptor, name_save):
    ...
    - One for 'ASER' and 'ACR'
    
    Parameters:
    - df (pd.DataFrame): Data containing 'name', 'TUPOR', 'SESY', 'ASER', and 'ACR' columns.
    ...
    """
    df = df[['name', 'TUPOR', 'SESY', 'ASER', 'ACR']]
    # Ensure required columns exist in DataFrame
    required_columns = {'name', 'TUPOR', 'SESY', 'ASER', 'ACR'}
    ...
    axes[0].plot(df["name"], df["TUPOR"], marker='o', label="TUPOR")
    axes[0].plot(df["name"], df["SESY"], marker='o', label="SESY")
    axes[0].set_title(f"TUPOR and SESY trends ({type_split}, {type_scaf})", fontsize=14)
    axes[0].set_xlabel("Generator", fontsize=12)
    axes[0].set_ylabel("Value of metrics", fontsize=12)
    ...
    # Right plot for ASER and ACR
    axes[1].plot(df["name"], df["ASER"], marker='o', label="ASER")
    axes[1].plot(df["name"], df["ACR"], marker='o', label="ACR")
    axes[1].set_title(f"ASER and ACR trends ({type_split}, {type_scaf})", fontsize=14)
    axes[1].set_xlabel("Generator", fontsize=12)
    axes[1].set_ylabel("Value of metrics", fontsize=12)
...
def plot_subset_trends(type_scaf, subsets_list, receptor, name_save):
    """
    ...
    for type_split in ['dis', 'sim']:
        for subset in subsets_list:
            try:
                file_path = f'data/results/{receptor}/{type_scaf}_scaffolds/{type_split}/mean_{type_scaf}_{type_split}{subset}.csv'
    ...
    x_labels = set(df_all['name'].str.replace('_mean', '').tolist())
    x_positions = range(len(x_labels)) 
...
def plot_trends_base(scaffolds=['csk', 'murcko'], subsets=['dis', 'sim'], receptor='', name_save=''):
    """
    Function to plot trends for given scaffolds and subsets.
    
    Arguments:
    scaffolds -- List of scaffold types to consider (default is ['csk', 'murcko'])
    subsets -- List of subsets to consider (default is ['dis', 'sim'])
    """
    # Initialize an empty DataFrame to combine the data
    df_all = pd.DataFrame()
    
    # Load data for each scaffold and subset type
    for type_scaf in scaffolds:
        for type_split in subsets:
            # Load the data for the given scaffold and split type
            df = pd.read_csv(f'data/results/{receptor}/{type_scaf}_scaffolds/{type_split}/mean_{type_scaf}_{type_split}.csv')
            df = df[['name', 'TUPOR', 'SESY', 'ASER', 'ACR']]
            df['Scaffold'] = type_scaf  # Add a column for the scaffold type
            df['Type_Split'] = type_split  # Add a column for the split type (DIS/SIM)
            df_all = pd.concat([df_all, df], ignore_index=True)
    ...
    for k, metrics_1 in enumerate(metrics):
        ...
        for i, scaffold in enumerate(scaffolds):
            ...
            for k, split in enumerate(subsets):
                subset_style_num = 0
                ...
                for col_metric in metrics_1:
                    ...
                    subset_style_num += 1
                l += 1
            axes[row, col].set_title(f"Trends for {metrics_1[0]} and {metrics_1[1]} ({scaffold})", fontsize=12)
