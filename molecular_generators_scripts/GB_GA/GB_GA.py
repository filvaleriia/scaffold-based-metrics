    ...
    if prune_population:
      ...
      for score, mol in zip(scores,population):
          try:
            ...
            print("Nepovedlo mol v sanitize")
    else:
...
def GA(args):
  ...
  for generation in range(generations):
    ...
    if scores[0] >= max_score:
      break
    result = [Chem.MolToSmiles(x) for x in population]
    df = pd.DataFrame(data=result)
    df.to_csv(f"cOS_GB_GA_mut_r_0.5_{type_split}_{number}_one_column_generation_{generation}.csv", index=False, header = False)
  return population
...
if __name__ == "__main__":
    ...
    results = GA((population_size, file_name,scoring_function,generations,mating_pool_size,mutation_rate, scoring_args, max_score, prune_population, seed))
    after = time.perf_counter()

    out = open(f"time_sheeet.txt","a")
    out.write(f"Used parameters: {population_size},{file_name},{scoring_function},{generations},{mating_pool_size}, {mutation_rate}, {scoring_args}, {max_score}, {prune_population}, {seed}.\n Time : {after-before} seconds \n\n\n")
    out.close()
