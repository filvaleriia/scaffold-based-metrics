...
def run_addcarbon(smi_train, results_base,  number, type_split, seed=None):
    if seed is not None:
        np.random.seed(seed)
    ...
    sampled_smiles = add_carbon.generate(1000000)
    fn_sampled = str(dir_results / f'cOS_addcarbon_{type_split}_{number}.txt')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--smi_train",
        type=str,
        default=f'cIS_Leukocyte_elastase_{type_split}_{number}.csv',
        help="File containing a SMILES on each line.")
    parser.add_argument(
        "--results_base",
        type=str,
        default='adddcarbon_res_LE',
    ...
    for i in range(args.nruns):
        run_addcarbon(args.smi_train, args.results_base, number, type_split, seed=i)